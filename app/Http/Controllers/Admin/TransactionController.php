...
use App\Models\{AccountLog, Currency, CurrencyMatch, Transaction, TransactionComplete, TransactionIn, TransactionOut, Users, LeverTransaction, TransactionOrdercopy, UserReal, UsersWallet};
    ...
    public function cancel(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($id, $type) {
                ...
                $result = change_wallet_balance($user_wallet, 2, -$shoud_refund_number, AccountLog::TRANSACTIONIN_IN_DEL, "取消{$type_name}". $currency_match->symbol . ",解除锁定" . $currency_name, true);
                if ($result !== true) {
                    throw new \Exception($result);
                }
                $result = change_wallet_balance($user_wallet, 2, $shoud_refund_number, AccountLog::TRANSACTIONIN_IN_DEL, "取消{$type_name}". $currency_match->symbol .",退回" . $currency_name);
                // 删除该挂单
                $trade->delete();
            });
