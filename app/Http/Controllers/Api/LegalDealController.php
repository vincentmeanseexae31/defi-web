...
class LegalDealController extends Controller
{
    ...
    public function postSend(Request $request)
    {
        ...
        if (empty($pay_ways) || !is_array($pay_ways)) {
            return $this->error('请选择有效的收付款方式');
        }
        foreach($pay_ways as $v){
            if(!in_array($v,['bank','ali_pay','we_chat'])){
                return $this->error('付款方式参数值有误');
            }
        }
        ...
        try {
            ...
            $legal_deal_send->pay_ways=implode(',', $pay_ways);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function downSend(Request $request){
        $id = $request->get('id', 0);
        if (empty($id)) {
            return $this->error('参数错误');
        }
//        $send = C2cDealSend::find($id);
        $legalDealSend= LegalDealSend::find($id);
        if (empty($legalDealSend)) {
            return $this->error('无此记录');
        }
        DB::beginTransaction();
        try {
//            C2cDealSend::sendBack($id);
           $legalDealSend->is_shelves=2;
           $legalDealSend->save();
            DB::commit();
    ...
    public function upSend(Request $request){
        $id = $request->get('id', 0);
        if (empty($id)) {
            return $this->error('参数错误');
        }
    ...
    public function doDeal(Request $request)
    {
        ...
        try {
            ...
            if (empty($users_wallet)) {
                throw new \Exception('您无此钱包账号');
            }
            if (!empty($users_wallet->status)) {
                throw new \Exception('您的钱包已被锁定，请联系管理员');
            }
            ...
            if ($legal_deal_send->type == 'buy') { //求购
                ...
                $result = change_wallet_balance($users_wallet, 1, $number, AccountLog::LEGAL_DEAL_USER_SELL, '出售给商家法币:增加冻结', true);
                if ($result !== true) {
                    throw new \Exception($result);
                }
                //扣除手续费
                if (bc_comp($fee, '0') > 0) {
                    $result = change_wallet_balance($users_wallet, 1, -$fee, AccountLog::LEGAL_TRADE_FREE, '出售给商家法币,扣除手续费');
                    if ($result !== true) {
                        throw new \Exception($result);
                    }
                }
                ...
                if ($number > $legal_deal_send->surplus_number) {
                    throw new \Exception('您购买数量大于商家剩余数量!');
                } else {
                    $legal_deal_send->surplus_number = bc_sub($legal_deal_send->surplus_number, $number, 8);
                }
    ...
    public function backSend(Request $request)
    {
        ...
        try {
            ...
            if ($legal_send->is_shelves != 2) {
                throw new \Exception('请先下架交易,并等待该挂单下的所有交易完成后再做操作');
            }
            $has_notcomplete_deal = LegalDeal::where('legal_deal_send_id', $id)->whereNotIn('is_sure', [1, 2])->exists();
            if ($has_notcomplete_deal) {
                throw new \Exception('该发布信息下有交易正在进行中,无法撤回');
            }
            if (LegalDeal::where('legal_deal_send_id', $id)->exists()) {
                throw new \Exception('该发布信息有交易产生,无法删除');
            }
            $user_id = Users::getUserId();
            $seller = Seller::where('user_id', $user_id)
            ...
            if ($legal_send->type == 'sell') {   //如果商家发布出售信息
                if ($seller->lock_seller_balance < $legal_send->surplus_number) {
                    throw new \Exception('对不起，您的商家账户冻结资金不足');
                }
