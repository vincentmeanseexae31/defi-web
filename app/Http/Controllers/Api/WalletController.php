...
class WalletController extends Controller
{
    ...
    public function wallet($symbol)
    {
        $user_id = Users::getUserId(); 
        if (empty($user_id)) {
            return $this->error('参数错误');
        }
        $coin=Currency::where('symbol',$symbol)->first();
 
        $wallet=UsersWallet::where('user_id', $user_id)->where('currency',$coin['id'])->first();
        $result=[
            'symbol' => $coin['currency_name'],
            'fullName' => $coin['currency_name'],
            'icon' => $coin['currency_logo'],
            'showPrecision' =>$coin['showPrecision'],// $item['currency_decimal'],
            'normalBalance' => $wallet['change_balance'],
            'frozenBalance' => $wallet['lock_change_balance'],
            'priceUsd' => $wallet['usdt_price'],
            'priceCny' => $wallet['cny_price'],
        ];       
    ...
    public function walletList(Request $request)
    {
        ...
        $list= [];
        foreach($change_wallet['balance'] as $item)
        {
            // $total=$item['change_balance']+
            $list[]=[
                'symbol' => $item['currency_name'],
                'fullName' => $item['currency_name'],
                'icon' => $item['currency_logo'],
                'showPrecision' =>4,// $item['currency_decimal'],
                'normalBalance' => $item['change_balance'],
                'frozenBalance' => $item['lock_change_balance'],
                'priceUsd' => $item['usdt_price'],
                'priceCny' => $item['cny_price'],
              ] ;         
              $totalPrice['usdt_price']+=$item['usdt_price'];
              $totalPrice['cny_price']+=$item['cny_price'];
        }
        
        return $this->success(null,['list' => $list,'totalCnyAmount'=>$totalPrice['cny_price'],  'totalUsdAmount'=>$totalPrice['usdt_price']]);
    }


    public function currencyTips($coin){
        $coin=Currency::where('symbol',$coin)->first();
        $tips=$coin['tips'];
        ...
        return $this->success(null,$tips);
    }

    public function getWithdrawConfig($symbol){
        $coin=Currency::where('symbol',$symbol)->first();         
        $result=[
            'noAuthDayMaxWithdraw' => 0.0,
            'dayMaxWithdraw' => 0.0,
            'fee' => $coin['rate'],
            'maxWithdraw' =>  $coin['max_number'],
            'minWithdraw' =>  $coin['min_number'],
            'showPrecision' => $coin['showPrecision']
        ];
        return $this->success(null,$result);
    }
    ...
    public function currencyList()
    {
        ...
        foreach($currency as $item)
        {
            $result[]=[
                ...
                'showPrecision' => $item['showPrecision'],
                'icon' => $item['logo'],
                'isOpen' => 1,
                'isDeposit' => 1,
                'isWithdraw' => 1,
                'isMain' => 1,
                'depositConfirm' => 0,
                'maxWithdraw' => $item['max_number'],
                'minWithdraw' => $item['min_number'],
                'dayMaxWithdraw' => 0.0,
                'noAuthDayMaxWithdraw' => 0.0,
                'withdrawFee' => $item['rate'],
                'depositFee' => 0.0,
    ...
    {
        $user_id = Users::getUserId();
        $currency=Currency::where('symbol',Input::get("symbol", 0))->first();
        $currency_id = $currency['id'];
        $number = $request->input("amount", 0);
        $address = $request->input("address", '');
        $code = $request->input('capitalPasswd', '');
        ...
        try {
            ...
            throw_if(bc_comp($number, $wallet->{$field_name}) > 0, new \Exception($balance_type[2] . '余额不足'));
            ...
            $result = change_wallet_balance($wallet, $type, -$number, AccountLog::WALLETOUT, '申请提币扣除余额');
            if ($result !== true) {
                throw new \Exception($result);
            }

            $result = change_wallet_balance($wallet, $type, $number, AccountLog::WALLETOUT, '申请提币冻结余额', true);
            if ($result !== true) {
                throw new \Exception($result);
            }
