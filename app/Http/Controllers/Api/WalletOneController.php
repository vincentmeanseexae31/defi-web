...
class WalletOneController extends Controller
{
    ...
    public function moneyRechange($symbol){
        $user_id = Users::getUserId();
        if(empty($user_id)||empty($symbol)) return $this->error('参数错误');

        $currency=Currency::where('symbol',$symbol)->first();
 
        $userWallet = UsersWallet::where('user_id',$user_id)->where('currency',$currency->id)->first(); 
     
        $address=$userWallet['address'];
        if($currency['make_wallet']==2)
        {
            $address=$currency['total_account'];
        }
        $result=[
            'symbol' => $currency['symbol'],
            'address' => $address,
            'fee' => $currency['rate'],
            'showPrecision' => $currency['showPrecision'],
            'chain' => $currency['type'],
            'chains' => [],        
        ];
