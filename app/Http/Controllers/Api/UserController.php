...
use App\Models\{AppApi, UserChat, Users, UserReal, Token, AccountLog, UsersWallet, UsersWalletcopy, Currency, InviteBg, Setting, UserCashInfo, ExchangeShiftTo};
use App\DAO\RPC;
...
class UserController extends Controller
{
    ...
    {
        $subQuery= DB::table('users as a')->join('users as b','a.id','=','b.parent_id')
        ->groupBy('a.id')->select('a.id as uid','a.account_number as username',DB::raw( 'count(1) as children_count'))->toSql();    
        $query=DB::table(DB::raw("({$subQuery}) as sub") );
        $data=$query->orderByDesc('children_count')->limit(20)->get()->toArray();     
        $result=[];  
        ...
        {         
            ...
            $obj['invitUid1']=$item->children_count;
            $obj['username']= mb_substr($item->username, 0, 3) . '******' . mb_substr($item->username, -2, 3);
...
//        $user_id = Users::getUserId();
    ...
    public function realName()
    {
        ...
        //校验  身份证号码合法性
        $is_authRealName=Setting::getValueByKey('is_authRealName',0);
        if($is_authRealName == 1){
            $res=self::authRealName($name,$card_id);
            if($res !== true){
        ...
        try {
            ...
            DB::commit();
            return $this->success('实名认证申请已提交，等待审核!');
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->error($e->getMessage());
        }
    ...
    //验证身份证与姓名的一致性
    public  static function authRealName($name,$card_id){
        $url = 'http://op.juhe.cn/idcard/query';

        $key='0ad0a29377cba1af2c875b722efd353f';
