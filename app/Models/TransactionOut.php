...
class TransactionOut extends Model
{
    ...
    public static function transaction($in, $num, $user, &$wallet, $legal_id, $currency_id)
    {
        ...
        try {
            ...
            CurrencyQuotation::updateTodayPriceTable($data,$symbol);//更新今日价格表
            //推送K线行情
...
//            //chinpoooo
//            MarketHour::batchEsearchMarket($complete->currency_name, $complete->legal_name,
//                $complete->price, now()->timestamp,$market_hour->number);
            MarketHour::batchWriteEsearchMarket($complete->currency_name, $complete->legal_name,$currency_id,$legal_id,now()->timestamp);
//            $matchEngin=new MatchEngine();
//            $matchEngin->sendMarket($kline_data, $complete->number);
