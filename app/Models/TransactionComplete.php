...
class TransactionComplete extends Model
{
    ...
    public static function getComplete($user_id,$check_money)
    {
        if (empty($user_id)) {
            return null;
        }
        $start_time = strtotime(date('Y-m-d', strtotime('-30 day')));
        return self::where("create_time", ">=", $start_time)
            ->where("user_id", $user_id)
//            ->where('price', $check_money)
            ->get(['price', 'number', 'create_time']);
    }
