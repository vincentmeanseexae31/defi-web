...
class News extends Model
{
    ...
    public static function getHelpNewsList($lang){
        $list= self::where('c_id',11)->where('lang',$lang)->orderBy('sorts', 'desc')->get()->toArray();
        $new_list=[];
    ...
    public static function getNoticeList($lang){
        $list= self::where('c_id',4)->where('lang',$lang)->orderBy('sorts', 'desc')->get()->toArray();
        $new_list=[];
        foreach ($list as $item) {
            array_push($new_list,[
                'id'=>$item['id'],
