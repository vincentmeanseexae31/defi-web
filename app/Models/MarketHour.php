...
class MarketHour extends Model
{
    ...
    public static function batchEsearchMarket($base_currency, $quote_currency, $price, $time,$amount)
    {
        $types = [
            '1min' => 5,
            '5min' => 6,
...
//                    unset($data['vol'], $data['amount']); //不影响成交数量
    ...
    public static  function batchWriteEsearchMarket($base_currency,$quote_currency,$currency_id,$legal_id,$time){
        $types = [
            '1min' => 5,
            '5min' => 6,
            '15min' => 1,
        ...
        foreach ($periods as $key => $period) {
            ...
            $convert_time = self::formatTimeline($type, $time);
            try {
                $market_hour= self::getTimelineInstance($type, $currency_id, $legal_id, 1, $time);
//                $kline_data = [
//                    'type' => 'kline',
//                    'period' => $period,
...
//                ];
                ...
                $kline_data = [
                    ...
                    'low' => $market_hour->mminimum,
                    'vol' => $market_hour->number,
                    'amount' => $market_hour->number,
                ];

                self::setEsearchMarket($kline_data);
            } catch (\Exception $e) {
