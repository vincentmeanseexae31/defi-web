...
{{--        <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加数据','{{url('admin/market_add')}}')">添加产品</button>--}}
{{--    </div>--}}
    <button class="layui-btn layui-btn-normal layui-btn-radius" id="add_mining_machine">添加产品</button>
...
@section('scripts')
    <script>
        layui.use(['table','form','layer'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('admin/financial/list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'ID', Width:60, sort: true}
                    ,{field: 'financial_name', title: '产品名字', Width:80}
                    // ,{field: 'subtitle', title: '副标题', Width:80}
                    ,{field: 'describe', title: '产品描述', minWidth:80}
                    ...
                    ,{field: 'sorts', title: '排序', minWidth:80}
            ...
            form.on('switch(is_newuser)', function(obj){
                ...
                console.log(id);
                $.ajax({
                    url:'{{url('admin/financial/newuser')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    ...
                    {{--layer_show('编辑理财产品','{{url('admin/financial/add')}}?id='+data.id);--}}
                    var index = layer.open({
                            title:'编辑理财产品'
                            ,type:2
                            ,content: '/admin/financial/add?id='+data.id
                            ,area: ['800px', '600px']
                            ,maxmin: true
                            ,anim: 3
                        });
