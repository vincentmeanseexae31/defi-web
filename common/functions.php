 ...
 * @param integer          $balance_type     1.理财钱包,2.可提现钱包,3.杠杆交易,4.doge币余额
 ...
 */
function change_wallet_balance(&$wallet, $balance_type, $change, $account_log_type, $memo = '', $is_lock = false, $from_user_id = 0, $extra_sign = 0, $extra_data = '', $zero_continue = false, $overflow = false,$group_id='')
{
    //为0直接返回真不往下再处理
    ...
    $param = compact('balance_type', 'change', 'account_log_type', 'memo', 'is_lock', 'from_user_id', 'extra_sign', 'extra_data', 'zero_continue', 'overflow','group_id');
    try {
        if (!in_array($balance_type, [1, 2, 3,4])) {
        ...
        DB::transaction(function () use (&$wallet, $param) {
            ...
            /**
             * @var \App\Models\UsersWallet $wallet
             */
            ...
            if (bc_comp($after, '0') < 0 && !$overflow) {
                throw new \Exception('钱包余额不足'.$field);
            }
            $now = time();
            AccountLog::unguard();
